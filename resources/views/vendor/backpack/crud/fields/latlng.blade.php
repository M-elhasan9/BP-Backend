@php
    ...
    if (isset($entry) && $entry[$attr]!=null){
     ...
     $lng =  $entry[$attr]->lng ;
        ...
        {
          $lat =  41.00504260845406;
          $lng = 29.012446403503418;
        }
...
<div class="form-group col-md-9">
    ...
    <div class="input-group">
        <input id="search_address" class="form-control" type="text"
               placeholder="{{ trans('backpack::crud.enter_location') }}"/>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @push('crud_fields_scripts')
        {{-- YOUR JS HERE --}}
            <script
                src="https://maps.googleapis.com/maps/api/js?v=3&libraries=places&key={{ $field['google_api_key'] }}"></script>
        <script>
            ...
            $(document).ready(function () {
                ...
                function initialize() {
                    ...
                    var latlng = (latlong == '') ? new google.maps.LatLng({{$lat}},{{$lng}}) : new google.maps.LatLng(latlong[0], latlong[1]);
                    setLatLong({{$lat}},{{$lng}});
